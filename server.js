...
apiRouter.post("/auth/login", async (req, res) => {
  try {
    ...
    console.log(`Login attempt for: ${email} at ${(/* @__PURE__ */ new Date()).toISOString()}`);
    if (!email || !password) {
...
apiRouter.get("/admins", authenticate, (req, res) => {
  const admins = getAdmins();
  res.json(admins.map((a) => ({ email: a.email })));
});
...
apiRouter.post("/admins/remove", authenticate, (req, res) => {
  ...
  admins = admins.filter((a) => a.email.toLowerCase().trim() !== targetEmail);
...
apiRouter.post("/upload", authenticate, (req, res, next) => {
  ...
  console.log("Main upload handler reached. Files count:", req.files?.length);
  try {
    ...
    const uploadPromises = files.map(async (file) => {
      try {
        ...
        const result = await cloudinary.uploader.upload(file.path, {
          ...
          chunk_size: 6e6
          // 6MB chunks for better video handling
        });
        ...
        const code = uploadError.http_code || "N/A";
...
apiRouter.get("/gallery", async (req, res) => {
  try {
    ...
    try {
      const result = await cloudinary.search.expression("folder:gallery").sort_by("created_at", "desc").max_results(30).execute();
      ...
      const combined = [...images.resources, ...videos.resources].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
